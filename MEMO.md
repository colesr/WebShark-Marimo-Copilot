...
Three real datasets now: two with leaks (temporal + post-outcome) it caught with zero heuristic help, one clean set it correctly cleared. The audit isn't pattern-matching a planted clue, and it isn't trigger-happy. The one remaining honest caveat is **latency, not accuracy**: the leak/clean runs took ~75-85s at `effort="high"` (vs the ~37s medium default) — a real hit on Q1. Next time, run these at medium; the Favorita effort-sweep already showed medium holds the catch strength, so high is probably just paying for thinking budget the audit doesn't need.
